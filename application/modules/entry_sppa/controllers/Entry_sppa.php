...
class Entry_sppa extends CI_controller
{
  ...
  public function tampil_data_entry()
  {
    ...
    foreach ($list as $o) {
        ...
        $tbody[]    = number_format($o['total_sum_insured'],0,'.','.');
  ...
  public function tampil_edit_sppa($id_sppa)
  {
      ...
      $cari1     = $this->entry_sppa->get_sppa($id_sppa)->row_array();

      // insert ke tr_premi yang belum ada coverage 
      $list_cov = $this->cari_yang_beda_coverage($cari1['id_lob'], $id_sppa);

      if (!empty($list_cov)) {

        $tot_rate_premi = $cari1['total_rate_akhir_premi'];
        $gross_premi    = $cari1['gross_premi'];

        $total_diskon       = $cari1['total_diskon'];
        $total_akhir_premi  = $cari1['total_akhir_premi'];
        $total_tagihan      = $cari1['total_tagihan'];
        $biaya_admin        = $cari1['biaya_admin'];

        $tot_nominal  = 0;
        $tot_rate     = 0;

        foreach ($list_cov as $lc) {

          $cr = $this->entry_sppa->cari_data('coverage', ['id_coverage' => $lc])->row_array();

          if (!empty($cr)) {
            $tsi  = $cari1['total_sum_insured'];
            $rate = $cr['rate'];

            $nominal = $tsi * ($rate / 100);

            $data_cov[] = [ 'id_coverage'        => $lc,
                            'rate'               => $rate,
                            'nominal'            => 0,
            ...
            $tot_nominal  = $tot_nominal + $nominal;
          }
            
        }

        $this->db->insert_batch('tr_premi', $data_cov);

        $u_gross_premi    = $gross_premi + $tot_nominal;
        $u_tot_rate_premi = $tot_rate_premi + $tot_rate;

        $u_tot_akhir_premi  = $u_gross_premi - $total_diskon;
        $u_tot_tagihan      = $u_tot_akhir_premi + $biaya_admin;

        // ubah nilai akhir total
        // $dt_ubah = [ 'total_rate_akhir_premi' => $u_tot_rate_premi,
        //               'gross_premi'            => $u_gross_premi,
        //               'total_akhir_premi'      => $u_tot_akhir_premi,
        //               'total_tagihan'          => $u_tot_tagihan
        //           ];

        // $this->entry_sppa->ubah_data('tr_sppa_quotation', $dt_ubah, $where);
  ...
  public function simpan_semua_deklarasi_manual()
  {
    ...
    $this->db->trans_begin();
          ...
          $bln      = date("m", now('Asia/Jakarta'));
          $thn      = date("y", now('Asia/Jakarta'));

        // $nmr_invoice = "INV/$date/Entry/$random";

        // format invoice baru

          // $cr_inv_lob = $this->entry_sppa->cari_data_order('tr_sppa_quotation', ['id_lob' => $id_lob, 'no_invoice_entry !=' => ''], 'add_time', 'desc')->row_array();
          $cr_inv_lob = $this->entry_sppa->cari_data_order('tr_sppa_quotation', ['no_invoice_entry !=' => ''], 'id_sppa_quotation', 'desc')->row_array();
          $cr_lob     = $this->entry_sppa->cari_data('m_lob', ['id_lob' => $id_lob])->row_array();
          
          if (!empty($cr_inv_lob)) {
            $no_invoice_last = $cr_inv_lob['no_invoice_entry'];

            // penambahan increment nomer
            $letak_slash = strpos($no_invoice_last,"/");
            $urutan_last = substr($no_invoice_last,0, $letak_slash);

            $nmr    = (int) $urutan_last + 1;

            $nomer  = str_pad($nmr, 6, "0", STR_PAD_LEFT);

            // npmer invoice
            $nmr_invoice = $nomer."/LGW-INV/".$cr_lob['singkatan']."/".$bln."/".$thn;
            
          } else {
            $nmr_invoice = "000001/LGW-INV/".$cr_lob['singkatan']."/".$bln."/".$thn;
          }
          ...
          $cr_app = $this->entry_sppa->cari_data('tr_approve_sppa', $where)->row_array();
  ...
  // rubah nomer invoice
  public function rubah_invoice()
  {
    $this->db->where('status_aktif', 't');
    $this->db->order_by('id_sppa_quotation', 'asc');
    $list = $this->db->get('tr_sppa_quotation')->result_array();
    ...
    foreach ($list as $l) {
      ...
      $bln      = date("m", strtotime($l['add_time']));
      $thn      = date("y", strtotime($l['add_time']));
      ...
      // format invoice baru
        ...
        if ($i == 1) {
          ...
          // $cr_inv_lob = $this->entry_sppa->cari_data_order('tr_sppa_quotation', ['id_lob' => $id_lob, 'no_invoice_entry !=' => ''], 'id_sppa_quotation', 'desc')->row_array();
          $cr_inv_lob = $this->entry_sppa->cari_data_order('tr_sppa_quotation', ['no_invoice_entry !=' => ''], 'id_sppa_quotation', 'desc')->row_array();
        ...
        $this->entry_sppa->ubah_data('tr_sppa_quotation', ['no_invoice_entry' => $nmr_invoice], ['id_sppa_quotation' => $l['id_sppa_quotation']]);
  ...
  public function cari_yang_beda_coverage($id_lob, $id_sppa)
  {
    $cov = $this->entry_sppa->cari_data('coverage', ['id_lob' => $id_lob])->result_array();

    foreach ($cov as $c) {
      $list_cov[] = $c['id_coverage'];
    }

    $this->db->join('coverage c', 'c.id_coverage = t.id_coverage', 'inner');
    $this->db->where('c.id_lob', $id_lob);
    $this->db->where('t.id_sppa_quotation', $id_sppa);
    $premi = $this->db->get('tr_premi t')->result_array();

    foreach ($premi as $p) {
      $list_cov_p[] = $p['id_coverage'];
    }

    $result=array_diff($list_cov,$list_cov_p);
  ...
  public function cetak_invoice($id_sppa)
  {
    ...
    $wh_sob = $cari2['nama_sob'];
      ...
      switch ($cari2['id_sob']) {
          case 2:
            $this->db->select('m.nama_asuransi as nama, m.alamat, m.telp, k.kota');
            $this->db->join('m_kota k', 'k.id_kota = m.id_kota', 'inner');
            $this->db->where('m.id_asuransi', $wh_sob);
            $data_sob = $this->db->get('m_asuransi m')->row_array();
            break;
          case 3:
            $this->db->select('m.nama_nasabah as nama, m.alamat_rumah as alamat, m.telp, k.kota');
            $this->db->join('m_kota k', 'k.id_kota = m.id_kota', 'inner');
            $this->db->where('m.id_nasabah', $wh_sob);
            $data_sob = $this->db->get('m_nasabah m')->row_array();
            break;
          case 4:
            $this->db->select('m.nama, m.alamat, m.telp, k.kota');
            $this->db->join('m_kota k', 'k.id_kota = m.id_kota', 'inner');
            $this->db->where('m.id_agent', $wh_sob);
            $data_sob = $this->db->get('m_agent m')->row_array();
            break;
          case 6:
            $this->db->select('m.nama, m.alamat, m.telp, k.kota');
            $this->db->join('m_kota k', 'k.id_kota = m.id_kota', 'inner');
            $this->db->where('m.id_direct', $wh_sob);
            $data_sob = $this->db->get('m_direct m')->row_array();
            break;
          case 5:
            $this->db->select('m.nama, m.alamat, m.telp, k.kota');
            $this->db->join('m_kota k', 'k.id_kota = m.id_kota', 'inner');
            $this->db->where('m.id_business_partner', $wh_sob);
            $data_sob = $this->db->get('m_business_partner m')->row_array();
            break;
          case 7:
            $this->db->select('m.nama, m.alamat, m.telp, k.kota');
            $this->db->join('m_kota k', 'k.id_kota = m.id_kota', 'inner');
            $this->db->where('m.id_loss_adjuster', $wh_sob);
            $data_sob = $this->db->get('m_loss_adjuster m')->row_array();
            break;
        }
