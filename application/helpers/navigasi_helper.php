      ...
      if ($row['link'] == '#') {
        ...
        $link = site_url($row['link']);
			...
			if(toogle($row['id'],$user_id,$wh) > 0){
				...
				$menunav .= '<a href="'.$link.'"><i class="'.$row['icon'].'"></i>'.$row['nama_menu'].'<i class="mdi mdi-chevron-down mdi-drop"></i></a>';
				$menunav .=	formatTree($row['id'],$user_id,$wh);
				$menunav .= "</li>";
			} else {
				$menunav .= "<li class='has-submenu da'>";
				$menunav .= '<a href="'.$link.'"><span class="ml-3"> <i class="'.$row['icon'].' mr-2"></i>'.$row['nama_menu'].'</span></a>';
				$menunav .= "</li>";
			}
			$i++;
		}
      ...
      if ($item['link'] == '#') {
        ...
        $link = site_url($item['link']);
			...
			if(toogle($item['id'],$user_id,$wh) > 0){
				...
				$menunav .= '<a href="'.$link.'">'.$item['nama_menu'].'</a>';
